...
namespace Engine::Rendering
{
        ...
        : m_display(XOpenDisplay(nullptr)), m_window(0), m_screen(DefaultScreen(m_display)), m_isRunning(false)
    {
        ...
        {
            std::cerr << "[Window] Failed to open X display." << std::endl;
            m_isRunning = false;
            return;
        }
        ...
        while (XPending(m_display) > 0)
        {
            ...
            switch (event.type)
            {
                ...
                case KeyPress:
                {
                    ...
                    auto mapKey = [](KeySym s) -> Input::Key {
                    ...
                    Input::Key k = mapKey(sym);
                    if (k != Input::Key::Unknown) Input::SetKey(k, true);
                ...
                case KeyRelease:
                {
                    ...
                    Input::Key k = mapKey(sym);
                    if (k != Input::Key::Unknown) Input::SetKey(k, false);
                    break;
                }
